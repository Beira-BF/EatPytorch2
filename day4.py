...
# torch.utils.data.Dataset是一个抽象类，用户想要加载自定义的数据只需要继承这个类，并且复写其中的两个方法即可：
...
# 此处介绍一种类形式的训练循环。
# 我们仿照Kera定义了一个高阶的模型接口Model，实现fit, validate, predict, summary方法，相当于用户自定义高阶API。
...
# 第50天开始新增确诊将为0，第45天对应3月10日，也就是5天后，即预计3月15日新增确诊降为0
# 注：该预测偏乐观
