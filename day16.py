...
# Pytorch中的损失函数一般在训练模型的时候指定。
# 注意Pytorch中内置的损失函数的参数和tensorflow不同，是y_pred在前，y_true在后，而Tensorflow是y_true在前，y_pred在后。
# 对于回归模型，通常使用的内置损失函数是均方损失函数nn.MSELoss.
# 对于二分类模型，通常使用的二元交叉熵损失函数nn.BCELoss（输入已经是sigmoid激活函数之后的结果）或者nn.BCEWithLogitsLoss（输入
# 尚未经过nn.Sigmoid激活函数）。
# 对于多分类模型，一般推荐使用交叉熵损失函数nn.CrossEntropyLoss.(y_true需要是一维的，是类别编码。y_pred未经过nn.Softmax激活。）
# 此外，如果多分类的y_pred经过了nn.LogSoftmax激活，可以使用nn.NLLLoss损失函数（The negative log likelihood loss).这种方法和直接
# 使用nn.CrossEntropyLoss等价。
...
# 如：nn.BCE和F.binary_cross_entropy都是二元交叉熵损失函数，前者是类的实现形式，后者是函数的实现形式。
...
# nn.CrossEntropyLoss（交叉熵，用于多分类，要求label为稀疏编码，输入经过nn.LogSoftmax激活）
# nn.NLLLoss（负对数似然损失，用于多分类，要求label为稀疏编码，输入经过nn.LogSoftmax激活）
# nn.CosineSimilarity（余弦相似度，可用于多分类）
# nn.AdaptiveLogSoftmaxWithLoss（一种适合非常多类别且类别分布不均衡的损失函数，会自适应地将多个小类别合成一个cluster）
...
# 从而让模型更加聚焦在正样本和困难样本熵。这就是为什么这个损失函数叫做Focal Loss。
...
# 可见 focal_loss让容易样本的权重衰减到原来的0.0005/0.1054 = 0.00474
# 而让困难样本的权重只衰减到原来的0.0866/0.6931=0.12486

# 因此相对而言，focal_loss可以衰减到容易样本的权重。
...
# Pytorch的优化器支持一种称之为Per-parameter option的操作，就是对每一个参数进行特定的学习率，权重衰减率指定，以满足更为细致的要求。

weight_parameter = [param for name, param in model.named_parameters() if "bias" not in name]
bias_params = [param for name, param in model.named_parameters() if "bias" in name]

optimizer = torch.optim.SGD([{'params': weight_parameter, 'weight_decay': 1e-5},
